...
export const EditCourt = ({ closeModal, courtDetail }: ChildProp) => {
  ...
  useEffect(() => {
    ...
    setValue("normalcost.time.from", courtDetail.normalcost.time.from);
    setValue("normalcost.time.to", courtDetail.normalcost.time.to);
    if (courtDetail && courtDetail.specialcost) {
      setValue(
        "specialcost.category" as
          ...
          | "normalcost.day.from"
          | "normalcost.day.to"
          | "normalcost.time.from"
          | "normalcost.time.to",
        courtDetail.specialcost?.category
      );
      setValue(
      ...
      setValue(
        "specialcost.diff.from" as
          ...
          | "normalcost.time.from"
          | "normalcost.time.to",
        courtDetail.specialcost?.diff.from
      );
      setValue(
        "specialcost.diff.to" as
          ...
          | "normalcost.time.from"
          | "normalcost.time.to",
        courtDetail.specialcost?.diff.to
      );
      setSpOption(true);
    }
  }, [courtDetail]);
  ...
  return (
    <form
      className="w-full  relative max-h-[690px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 p-6 bg-white rounded-md transition-all duration-300"
      onSubmit={handleSubmit(submitCourtForm)}
    >
      <div className="w-full flex justify-start">
        <span className="font-semibold">Edit Synthetic court</span>
      </div>
      ...
      <div className="w-full px-3 py-2 border rounded-md mt-3">
        ...
        <div className="w-full grid grid-cols-1 gap-3 md:grid-cols-2 mt-2">
          <div className="flex flex-col">
            ...
            >
              <SelectTrigger className="w-full">
                <SelectValue placeholder={watch("normalcost.day.from")} />
              </SelectTrigger>
              <SelectContent>
                <SelectGroup>
                  {days.map((val) => {
          ...
          <div className="flex flex-col">
            ...
            >
              <SelectTrigger className="w-full">
                <SelectValue placeholder={watch("normalcost.day.to")} />
              </SelectTrigger>
              <SelectContent>
                <SelectGroup>
                  {days.map((val) => {
      ...
      <div className="flex items-center space-x-2 w-full mt-3">
        <Switch
          id="airplane-mode"
          defaultChecked={spcialCostoption}
                  ...
                  <SelectValue
                    placeholder={
                      (watch(
                        ...
                        : "Select type of category") as ReactNode
                    }
                  />
                </SelectTrigger>
                <SelectContent>
      ...
      {/* end */}
      <div className="w-full mt-4  bottom-0   left-0 z-10">
